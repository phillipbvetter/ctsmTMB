...
apply_algebraics_and_define_trans_equations = function(self, private) {
  ...
  # extract rhs's
  alg.rhs = lapply(private$model$alg.eqs, function(x) x$rhs)
  sys.rhs = lapply(private$model$sys.eqs,function(x) x$rhs)
  obs.rhs = lapply(private$model$obs.eqs,function(x) x$rhs)
  obs.var.rhs = lapply(private$model$obs.var,function(x) x$rhs)
  ...
  # system
  # for(i in seq_along(private$model$sys.eqs)) {
  for(i in seq_along(sys.rhs)) {
    temp.form = private$model$sys.eqs[[i]]$form
    temp.form[[3]] = sys.rhs[[i]] #[[3]] is rhs of a formula
    temp.list = list(form=temp.form, name=private$names$states[i])
    private$add_trans_systems(temp.list)
  }

  # observations
  # for(i in seq_along(private$model$obs.eqs)) {
  for(i in seq_along(obs.rhs)) {
    temp.form = private$model$obs.eqs[[i]]$form
    temp.form[[3]] = obs.rhs[[i]] #[[3]] is rhs of a formula
    temp.list = list(form=temp.form, name=private$names$obs[i])
    private$add_trans_observations(temp.list)
  }

  # observation variances
  # for(i in seq_along(private$model$obs.var)) {
  for(i in seq_along(obs.var.rhs)) {
    temp.form = private$model$obs.var[[i]]$form
...
calculate_diff_terms = function(self, private) {
  ...
  for (i in seq_along(private$model$sys.eqs.trans)) {
    private$model$diff.terms[[i]] = lapply(private$model$diff.processes, function(x) ctsmTMB_Deriv(f=private$model$sys.eqs.trans[[i]]$rhs, x=x))
    names(private$model$diff.terms[[i]]) = private$model$diff.processes
  }
  names(private$model$diff.terms) = private$names$states

  # dfdx
  for(i in seq_along(private$model$sys.eqs.trans)){
    private$model$diff.terms.drift[[i]] = lapply(private$names$states, function(x) ctsmTMB_Deriv(f=private$model$sys.eqs.trans[[i]]$diff.dt, x=x))
    names(private$model$diff.terms.drift[[i]]) = private$names$states
  }
  names(private$model$diff.terms.drift) = private$names$states

  # dhdx
  for(i in seq_along(private$model$obs.eqs.trans)){
    private$model$diff.terms.obs[[i]] = lapply(private$names$states, function(x) ctsmTMB_Deriv(f=private$model$obs.eqs.trans[[i]]$rhs, x=x))
    names(private$model$diff.terms.obs[[i]]) = private$names$states
  }
  names(private$model$diff.terms.obs) = private$names$obs
}
...
apply_lamperti = function(self, private) {
  ...
  transforms =  private$model$lamperti$transforms
  states = private$model$lamperti$states
  ...
  for(i in seq_along(states)){
    ...
    dw.list = list(dw=parse(text=private$model$diff.processes[diffusion.id[i]])[[1]])

    # get drift and diffusion
    fg.list = list(
      f = private$model$diff.terms[[state]][["dt"]],
      g = private$model$diff.terms[[state]][[dw.list$dw]]
    )
  ...
  for(j in seq_along(private$model$obs.eqs)){

    # Get observation and variance rhs
    obs.rhs = private$model$obs.eqs.trans[[j]]$rhs
    obs.var.rhs = private$model$obs.var.trans[[j]]$rhs
