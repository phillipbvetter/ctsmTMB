---
title: "Extracting likelihood function, gradient and using custom optimizers"
author: ""
date: ""
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Extracting likelihood function, gradient and using custom optimizers}
...
In this document we show how to use the `constructNegLogLike` method to obtain function handlers for the objective function, gradient, and hessian (the latter is only avaible when there are no random effects in your model).
...
We use the common Ornstein-Uhlenbeck process to showcase the use of `constructNegLogLike`. 
...
The `constructNegLogLike` method allows you to retrieve the `nll` object that holds the negative log-likelihood, and its derivatives. The method takes arguments similar to those of `estimate`.

```{r}
nll = obj$constructNegLogLike(.data)
```
...
```{r remove files after vignette has been compiled, include=FALSE}
unlink(paste0(getwd(),"/ctsmTMB_cppfiles"), recursive=TRUE)
```
