...
```{r, echo=FALSE, fig.height=5,fig.width=9,out.width="100%", fig.align='center', fig.alt='The plot shows observations and input signal.'}
ggplot() + 
...
```{r, echo=FALSE, fig.height=5,fig.width=9,out.width="100%", fig.align='center', fig.alt='The plot shows a series of full predictions for different values of the theta parameter, against the observations.'}
t <- pred$states$t.j
latex.str <- lapply(
...
```{r}
pdf <- cbind(pred$states, y.data = pred$observations$y.data)
rmse <- data.frame(
  k.ahead = 0:pred.horizon,
  rmse = sapply(split(pdf, pdf$k.ahead), function(df) {
