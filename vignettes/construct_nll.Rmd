...
You can extract the parameter bounds specified when calling `setParameter()` method by using the `get_parameters` method (note that `nll$par` and `pars$initial` are identical).
